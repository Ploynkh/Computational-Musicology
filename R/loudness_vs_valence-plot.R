...
rnb_tracks <- bind_rows(rnb_90s, rnb_2020s) %>%
  ...
  drop_na(Loudness, Valence)

# ── Plot ──────────────────────────────────────────────────────────────────────
loudness_vs_valence <- ggplot(rnb_tracks, aes(x = Loudness, y = Valence, colour = Era)) +
  geom_point(size = 2.2, alpha = 0.65) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 1) +
  scale_colour_manual(values = era_colour) +
  labs(x = "Loudness (dB)", y = "Valence (0–1)") +
...
plot(loudness_vs_valence)

# ── Save ──────────────────────────────────────────────────────────────────────
ggsave("images/loudness_vs_valence.png",
       plot   = loudness_vs_valence,
       width  = 7,
       height = 5,
       dpi    = 300,
