...
To test whether the differences between the two eras are strong enough to support
classification, a k-nearest neighbours (kNN) classifier was trained to predict
era, 1990s or 2020s, from audio features alone. The model was evaluated using
5-fold cross-validation to ensure the results are not specific to a single
train-test split.

If the classifier performs well above chance (50%), it provides direct quantitative
evidence that the two eras are genuinely separable by their Spotify audio features.

![](images/confusion_matrix.png)

The confusion matrix shows how often the model correctly predicted each era.
Correct predictions appear along the diagonal. High accuracy on both classes
would confirm that 1990s and 2020s R&B occupy distinct regions of the audio
feature space supporting the central claim of this project.
